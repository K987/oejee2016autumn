...
public class RegisterFacade implements RegisterFacadeRemote, RegisterFacadeLocal {
     ...
     */
    @Override
    public List<RegisterCloseStub> getRegistersToClose(final Calendar day) throws FacadeException {
        LOG.info("getRegistersToClose invoked");
        List<RegisterCloseStub> rtrn = new ArrayList<>();
        try {
            rtrn = rConverter.to(rService.readRegisterClose(day));
            if (rtrn.isEmpty()) {
                LOG.info("register is empty");
                List<RegisterCloseStub> tmp = rConverter.to(rService.readAllWithLastClose());
     ...
     */
    @Override
    public void batchRegisterClose(final List<RegisterCloseStub> toClose) throws FacadeException {
        ...
        try {
            rService.createBatchRegisterClose(closes);
        } catch (PersistenceServiceException e) {
            LOG.error(e);
