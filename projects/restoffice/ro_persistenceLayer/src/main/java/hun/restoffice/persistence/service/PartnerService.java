...
public class PartnerService implements PartnerServiceLocal {
     ...
     */
    @Override
    public Partner read(final String partnerName) throws PersistenceServiceException {

        try {
            return entityManager.createNamedQuery(Partner.FIND_BY_NAME, Partner.class).setParameter(Partner.NAME, partnerName.toLowerCase().trim())
                    .getSingleResult();
     ...
     */
    @Override
    public List<Partner> readAll(final Boolean technical) throws PersistenceServiceException {
        List<Partner> rtrn = null;
        try {
            if (technical == null) {
                // return all
                rtrn = entityManager.createNamedQuery(Partner.FIND_ALL, Partner.class).setParameter(Partner.APPLY_CRITERIA, false)
                        .setParameter(Partner.IS_TECHNICAL, false).getResultList();
            } else {
                // return only the tecnical or non technical ones
                rtrn = entityManager.createNamedQuery(Partner.FIND_ALL, Partner.class).setParameter(Partner.APPLY_CRITERIA, true)
     ...
     */
    @Override
    public Partner update(final Partner partner) throws PersistenceServiceException {
    ...
    private int count(final Partner partner) throws PersistenceServiceException {
        try {
            return entityManager.createNamedQuery(Partner.COUNT, Long.class).setParameter(Partner.NAME, partner.getName().toLowerCase().trim())
     ...
     */
    @Override
    public Partner readById(final Integer id) throws PersistenceServiceException {
        try {
            return entityManager.createNamedQuery(Partner.READ_BY_ID, Partner.class).setParameter(Partner.ID, id).getSingleResult();
        } catch (NoResultException e) {
     ...
     * (non-Javadoc)
     *
     * @see hun.restoffice.persistence.service.PartnerServiceLocal#delete(java.lang.
     * Integer)
     */
    @Override
    public void delete(final Integer partnerId) throws PersistenceServiceException {
        Partner p = readById(partnerId);
